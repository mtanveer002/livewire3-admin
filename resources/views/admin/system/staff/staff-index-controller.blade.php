...
<div>
    <div class="text-right mb-4">
        <x-admin.components.button tag="a" href="{{ route('admin.system.staff.create') }}" wire:navigate>
            ...
            <div class="flex items-center space-x-4">
                <div class="grid grid-cols-12 w-full space-x-4">
                    <div class="col-span-8 md:col-span-8">
                        <x-admin.components.input.text wire:model.debounce.300ms="search" placeholder="{{ __('staff.index.search.placeholder') }}" />
                    </div>
                ...
                @forelse($staff as $staffMember)
                    <x-admin.components.table.row wire:loading.class.delay="opacity-50">
                        <x-admin.components.table.cell>
                            <img class="object-cover w-8 h-8 rounded-full" src="{{ $staffMember->profileImageUrl }}"
                        ...
                        <x-admin.components.table.cell>
                            @if (!$staffMember->deleted_at)
                                <a href="{{ route('admin.system.staff.show', $staffMember->id) }}" class="text-indigo-500 hover:underline" wire:navigate>
                                    {{ __('staff.index.action.edit') }}
                                </a>
                            @else
                                <button class="text-indigo-500 hover:underline" wire:click="restore('{{ $staffMember->id }}')">
                                    {{ __('staff.index.action.restore') }}
                                </button> |
                                <x-confirmation-modal model="forceDelete('{{ $staffMember->id }}')" />
                            @endif
                        </x-admin.components.table.cell>
