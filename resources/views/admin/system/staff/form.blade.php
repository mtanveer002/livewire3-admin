<div class="col-span-12 space-y-4">
    <x-admin.components.card heading="Basic Information">
        <div class="grid grid-cols-2 gap-4">
            <x-admin.components.input.group label="{{ __('inputs.firstname') }}" for="firstname" :error="$errors->first('staff.firstname')">
                <x-admin.components.input.text wire:model="staff.first_name" name="firstname" id="firstname"
                    :error="$errors->first('staff.firstname')" />
            </x-admin.components.input.group>
            <x-admin.components.input.group label="{{ __('inputs.lastname') }}" for="lastname" :error="$errors->first('staff.lastname')">
                <x-admin.components.input.text wire:model="staff.last_name" name="lastname" id="lastname"
                    :error="$errors->first('staff.lastname')" />
            </x-admin.components.input.group>
        </div>
        ...
        <x-admin.components.input.group label="{{ __('inputs.email') }}" for="email" :error="$errors->first('staff.email')">
            <x-admin.components.input.text wire:model="staff.email" type="email" name="email" id="email"
                :error="$errors->first('staff.email')" />
        </x-admin.components.input.group>
        ...
        <div class="grid grid-cols-2 gap-4">
            ...
            <x-admin.components.input.group label="{{ __('inputs.new_password_confirmation') }}"
                for="passwordConfirmation" :error="$errors->first('password_confirmation')">
                <x-admin.components.input.text wire:model="password_confirmation" type="password"
                    name="password_confirmation" id="passwordConfirmation" :error="$errors->first('passwordConfirmation')" />
            </x-admin.components.input.group>
        </div>
    ...
    <div class="px-4 py-3 text-right rounded shadow bg-gray-50 sm:px-6">
        <button type="submit"
            ...
            {{ __($staff->id ? 'settings.staff.form.update.btn' : 'settings.staff.form.create.btn') }}
        </button>
    </div>
            ...
            <div class="p-6 space-y-4 text-sm">
                <div class="grid grid-cols-12 gap-4">
                    <div class="col-span-12 md:col-span-6">
                        <strong>{{ __('settings.staff.form.danger.zone.label') }}</strong>
                        <p class="text-xs text-gray-600">{{ __('settings.staff.form.danger.zone.instructions') }}</p>
                    </div>
                    <div class="col-span-9 lg:col-span-4">
                        <x-admin.components.input.text type="email" wire:model.live="deleteConfirm" />
                    </div>
